...
# We will replace the entire while loop with the new Dynamic Time-Slicing algorithm.

start_marker = "    let segmentIndex = 1;"
...
new_logic = """
    ...
    for (let i = 0; i < prefetchSongs.length; i++) {
        ...
        if ((i + 1) % 2 === 0) {
            if (adCount < NUM_ADS && jtCount < NUM_JTS) {
                if ((i + 1) % 4 === 0) { // e.g. Song 4, Song 8 -> Drop Ad
                    // AD Insertion
                    ...
                    adCount++;
                } else { // e.g. Song 2, Song 6 -> Drop JT
                    // JT Insertion
    ...
    while (jtCount < NUM_JTS) {
         addElement('jocktalk', jt_dur_ms, "", { title: `JT${jtCount + 1}`, isEmptyPlaceholder: true });
         jtCount++;
    }
    while (adCount < NUM_ADS) {
         addElement('sweeper', AD_DURATION_MS, getSweeperByGenre(cityId), { title: "Radio Sweeper", isAd: true });
         adCount++;
    }
"""

# Extract the loop bounds to replace
try:
    ...
    with open("app/api/broadcast/generate-hour/route.ts", "w", encoding="utf-8") as f:
        f.write(new_content)
    print("Master Clock Time Slicing Refactored.")
