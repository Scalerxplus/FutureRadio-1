...
export async function searchAudiusTrack(query: string): Promise<AudiusTrack | null> {
  try {
    const host = await getAudiusHost();
    const encodedQuery = encodeURIComponent(query);
    const searchUrl = `${host}/v1/tracks/search?query=${encodedQuery}&app_name=${APP_NAME}&limit=1`;
